...
const MyProfile = () => {
  ...
  return userData && (
    <div className='max-w-lg flex flex-col gap-4 text-sm glass-card p-8 rounded-[2rem] mt-10 animate-fade-up'>
      ...
      {
        isEdit
        ? <input className='bg-slate-900/50 border border-white/10 text-white rounded-lg px-4 py-2 text-2xl font-bold max-w-60 mt-4 focus:ring-1 focus:ring-indigo-500 outline-none' type="text" value={userData.name} onChange={e => setUserData(prev => ({...prev, name: e.target.value}))}/>
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-4 mt-3 text-slate-300'>
          ...
          {
            isEdit
            ? <input className='bg-slate-900/50 border border-white/10 text-white rounded px-3 py-1 outline-none focus:border-indigo-500 max-w-52' type="text" value={userData.phone} onChange={e => setUserData(prev => ({...prev,phone: e.target.value}))}/>
