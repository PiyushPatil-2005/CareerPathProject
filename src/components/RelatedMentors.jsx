...
const RelatedMentors = ({ speciality, mentorId }) => {
  ...
  return (
    <div>
      <div className='flex flex-col items-center gap-4 my-16 text-slate-200 md:mx-10 animate-fade-up'>
        <h1 className='text-3xl font-bold glow-text text-white'>Top Mentors to Book</h1>
        <p className='sm:w-1/3 text-center text-slate-400'> Simply browse through our extensive list of trusted doctors.</p>
        <div className='w-full grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-6 pt-5 px-3 sm:px-0'>
          {relMentor.slice(0, 5).map((item, index) => (
            <div onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }} className='glass-card rounded-2xl overflow-hidden cursor-pointer hover:-translate-y-2 transition-all duration-300 group'>
              <div className="relative overflow-hidden h-64 bg-slate-800/50 border-b border-white/5">
                <img
                  src={item.image}
                  alt={item.name}
